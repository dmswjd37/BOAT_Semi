...
4. 회원 목록의 삭제를 클릭한 경우
 ...
 <script>
 	$(function() {
 		...
 		const message=["사원번호","이름","나이","여 또는 남"]
			...
			<div class="input-group">
				<select id="viewcount" name="search_field">
					...
					<input name="search_word" type="text" class="form-control"
						   placeholder="아이디 입력하세요"  value="${search_word}" style="border-color:#18a8f1;">
					<button class="btn btn-primary" type="submit" style="background-color:#18a8f1; border-color:#18a8f1;">검색</button>
			</div>
		</form>
			...
			<table class="table table-striped" style="font-weight: bold;">
				...
				<tbody>
					<c:forEach var="m" items="${totallist}">
						<tr>
							<td><a href="memberInfo.net?id=${m.empno}">${m.empno}</a></td>
							<td>${m.name}</td>
							<td><a href="memberDelete.net?id=${m.empno}">삭제</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
						...
						<c:forEach var="a" begin="${startpage}" end="${endpage}">
							<c:if test="${a == page}">
								<li class="page-item active">
									<a class="page-link" style="background-color:#18a8f1; border-color:#18a8f1;">${a}</a>
								</li>
							</c:if>
							<c:if test="${a != page }">
